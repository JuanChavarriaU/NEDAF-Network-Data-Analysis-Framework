...
logger = logging.getLogger("nedaf.network_visualization")
...
class NetworkVisualizationMod(QWidget):
    ...
    def on_visualization_finished(self):
        logger.info("Graph computation complete")
    ...
    def on_operation_changed(self, G: nx.Graph):
        ...
        try:
            ...
            logger.debug(f"Selected operation: {selected_operation}")
            if selected_operation in self.operation_to_function_map:
                ...
                logger.info(f"{selected_operation}: {result}")
