...
class ImportData(QWidget):
   ...
   def LoadData(self):
      ...
      try:
            if self.file.endswith('.csv') :
               ...
               df = pl.read_excel(self.file)
               
            elif self.file.endswith('.edges'):
               try:
                   df = pl.read_csv(self.file, separator=' ', has_header=False, new_columns=["source", "destination", "weight"])
               except Exception:
                   df = pl.read_csv(self.file, separator=' ', has_header=False, new_columns=["source", "destination"])
               ...
               if isinstance(matrixData, csr_matrix):
                   ...
                   df = pl.DataFrame({'source': rows, 'destination': cols, 'weight': weights})
               else:
                   df = pl.DataFrame({'source': rows, 'destination': cols})    
   ...
   def fill_data_table(self, df: pl.DataFrame):
       #vista previa del df
      preview_data = df.head(100).to_numpy()
